...
function ServiceCard({ service, index }: { service: typeof services[0], index: number }) {
    ...
    return (
        <motion.div 
            className="w-[85vw] sm:w-[400px] md:w-[450px] lg:w-[500px] flex-shrink-0 perspective-[1200px]"
            variants={cardVariants}
        >
            <motion.div 
                className="w-full aspect-[4/5] bg-[#3F352C] text-[#E8E3DA] flex flex-col justify-between p-8 md:p-12 relative overflow-hidden group cursor-pointer shadow-2xl transition-shadow hover:shadow-2xl hover:shadow-[#3F352C]/40 border border-[#3F352C]/10"
                onMouseMove={handleMouse}
                onMouseLeave={resetMouse}
                style={{ rotateX: y, rotateY: x, transformStyle: "preserve-3d" }}
            >
                {/* Large Circular Numerical Index */}
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-full border border-[#E8E3DA] flex items-center justify-center font-mono text-xl md:text-2xl z-10 transition-colors duration-500 group-hover:bg-[#E8E3DA] group-hover:text-[#3F352C]">
                    {service.id}
                </div>

                {/* Content */}
                <div className="flex flex-col z-10" style={{ transform: "translateZ(60px)" }}>
                    <h3 className="font-monument text-4xl md:text-5xl lg:text-6xl uppercase tracking-tighter leading-none mb-4 pointer-events-none transition-transform duration-500 group-hover:scale-105 origin-left">
                        {service.title}
                        <br/>
                        <span className="font-serif italic font-light lowercase text-3xl md:text-4xl lg:text-5xl tracking-normal opacity-80">{service.subtitle}</span>
                    </h3>
                    <div className="w-full h-px bg-[#E8E3DA]/20 my-6 transition-all duration-500 group-hover:w-[70%]"></div>
                    <p className="font-circular text-sm md:text-base lg:text-lg opacity-80 pointer-events-none group-hover:opacity-100 transition-opacity duration-300">
                        {service.desc}
                    </p>
                </div>
                
                {/* Box Art Internal Overlay for extra 3d depth */}
                <div className="absolute inset-0 border-[1px] border-[#E8E3DA]/20 m-4 md:m-6 pointer-events-none transition-transform duration-500 group-hover:scale-95"></div>
            </motion.div>
        </motion.div>
    );
}
...
export default function ServicesSection() {
    ...
    return (
        ...
        >
            ...
            <div className="w-full h-screen sticky top-0 flex flex-col justify-center overflow-hidden z-10 pt-20 md:pt-32">
                ...
                <div className="w-full h-full flex flex-col justify-start relative z-10">
                    ...
                    <div className="w-full px-4 md:px-12 lg:px-[172px] mb-12 md:mb-24">
